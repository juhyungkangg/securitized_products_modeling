...
def swap_rate(DF):
    sr = np.zeros(len(DF))
    for i in range(len(DF) - 2 * 10 * 12):
        sr[i] = 12 * (1 - DF.iloc[i + 10 * 12 - 1]) / np.sum(DF.iloc[i:i + 10 * 12])

    for i in range(len(DF) - 2 * 10 * 12, len(DF)):
        sr[i] = sr[i -1]
...
def psa_func(val=100, period=120):
    psa = np.linspace(0.2, 6, 30) * val / 100
    return np.append(psa, np.full(period-30, psa[-1]))


def factor_to_smm(factor):
    init_principal = 518318

    def obj_func(smm):
        pool_temp = PoolRefi(smm, maturity=360, wac=0.04070, wala=29, init_principal=init_principal)
        curr_curr_amt = pool_temp.BeginPrincipal[29]
...
class PassthroughRefi(object):
    ...
    def feed(self, pool):
        ...
        pool_df = pool.table[pool.wala:].reset_index(drop=True)
